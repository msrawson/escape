...
def draw_text(text: str, font_size: int) -> None:
    global screen, TEXT_OUTLINE, FONT_NAME, font_scale
    my_font = pygame.font.SysFont(FONT_NAME, font_size)
    lines = text.split("\n")
    line_height = font_size * font_scale * 1.1
...
def draw_timer() -> None:
    global minutes_left, clock_speed
    draw_background()
    minutes, seconds = divmod(int(minutes_left * 60), 60)
    draw_text(f"{minutes}:{seconds:02d}", 400)
...
def draw_hint() -> None:
    global hints, hint_number
    draw_background()
    if 1 <= hint_number <= len(hints):
        draw_text(hints[hint_number - 1], 220)
